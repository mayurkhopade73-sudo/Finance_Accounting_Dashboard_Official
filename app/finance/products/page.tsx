...
type AddForm = { name: string; category: string; price: string; stock: string; status: string };
type AddErrors = { name?: string; price?: string };

type EditErrors = { name?: string; price?: string };

const defaultAddForm: AddForm = { name: '', category: 'Software', price: '', stock: 'Unlimited', status: 'Active' };

// ─── Validation ───────────────────────────────────────────────────────────────

function validateAdd(form: AddForm): AddErrors {
...
function validateEdit(p: typeof initProducts[0]): EditErrors {
  const errs: EditErrors = {};
  if (!p.name.trim()) errs.name = 'Product name is required.';
  if (!p.price.trim()) errs.price = 'Price is required.';
  return errs;
}
...
export default function ProductsPage() {
  ...
  const handleAddChange = (field: keyof AddForm, value: string) => {
    const updated = { ...form, [field]: value };
    setForm(updated);
    if (addTouched[field]) setAddErrors(validateAdd(updated));
  };

  const handleAddBlur = (field: keyof AddErrors) => {
    setAddTouched(prev => ({ ...prev, [field]: true }));
    setAddErrors(validateAdd(form));
  };

  const handleAdd = () => {
    setAddTouched({ name: true, price: true });
  ...
  const handleEditChange = (field: keyof typeof initProducts[0], value: string) => {
    if (!editProduct) return;
  ...
  const handleEditBlur = (field: keyof EditErrors) => {
    ...
    setEditErrors(validateEdit(editProduct));
  };

  const handleEdit = () => {
    if (!editProduct) return;
    setEditTouched({ name: true, price: true });
    const errs = validateEdit(editProduct);
    setEditErrors(errs);
    if (Object.keys(errs).length > 0) return;
  ...
  return (
    <div className="p-6 space-y-6">
        ...
        <div className="p-4 border-b border-gray-100">
          <div className="relative max-w-sm">
            ...
            <input
              ...
              className="w-full bg-gray-50 border border-gray-200 rounded-lg pl-9 pr-3 py-2 text-sm text-gray-700 focus:outline-none"
            />
          </div>
        </div>
        <table className="w-full">
            ...
            {filtered.map(p => (
              <tr key={p.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3.5">
                  <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${p.status === 'Active' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-gray-100 text-gray-500'}`}>
                    {p.status}
                  </span>
                </td>
                <td className="px-4 py-3.5">
                  <button
                    onClick={() => { setEditProduct({ ...p }); setEditErrors({}); setEditTouched({}); }}
      ...
      {/* Add Product Modal */}
      <Modal open={showAdd} onClose={handleAddClose} title="Add Product">
        <div className="space-y-4">

          <FormField label="Product Name">
            <input
              className={errBorder(inputCls, !!(addTouched.name && addErrors.name))}
              value={form.name}
              onChange={e => handleAddChange('name', e.target.value)}
              onBlur={() => handleAddBlur('name')}
            />
            <Err msg={addTouched.name ? addErrors.name : undefined} />
          </FormField>

          <div className="grid grid-cols-2 gap-3">
            <FormField label="Category">
              <select className={selectCls} value={form.category} onChange={e => handleAddChange('category', e.target.value)}>
            ...
            <FormField label="Status">
              <select className={selectCls} value={form.status} onChange={e => handleAddChange('status', e.target.value)}>
          ...
          <FormField label="Price">
            <input
              className={errBorder(inputCls, !!(addTouched.price && addErrors.price))}
              value={form.price}
              onChange={e => handleAddChange('price', e.target.value)}
              onBlur={() => handleAddBlur('price')}
              placeholder="e.g. $2,400/yr"
            />
            <Err msg={addTouched.price ? addErrors.price : undefined} />
          </FormField>

          <FormField label="Availability">
            <select className={selectCls} value={form.stock} onChange={e => handleAddChange('stock', e.target.value)}>
          ...
          <div className="flex gap-3 pt-2">
            <button onClick={handleAddClose} className="flex-1 px-4 py-2 border border-gray-200 text-gray-600 rounded-lg text-sm hover:bg-gray-50">
              Cancel
            </button>
            <button onClick={handleAdd} className="flex-1 px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700">
              Add Product
            </button>
          </div>
      ...
      {/* Edit Product Modal */}
      <Modal open={!!editProduct} onClose={handleEditClose} title="Edit Product">
        {editProduct && (
          <div className="space-y-4">

            <FormField label="Product Name">
              <input
                className={errBorder(inputCls, !!(editTouched.name && editErrors.name))}
                value={editProduct.name}
                onChange={e => handleEditChange('name', e.target.value)}
                onBlur={() => handleEditBlur('name')}
              />
              <Err msg={editTouched.name ? editErrors.name : undefined} />
            </FormField>

            <FormField label="Price">
              <input
                className={errBorder(inputCls, !!(editTouched.price && editErrors.price))}
                value={editProduct.price}
                onChange={e => handleEditChange('price', e.target.value)}
                onBlur={() => handleEditBlur('price')}
              />
              <Err msg={editTouched.price ? editErrors.price : undefined} />
            </FormField>

            <FormField label="Status">
              <select className={selectCls} value={editProduct.status} onChange={e => handleEditChange('status', e.target.value)}>
                <option>Active</option>
                <option>Discontinued</option>
              </select>
            </FormField>
