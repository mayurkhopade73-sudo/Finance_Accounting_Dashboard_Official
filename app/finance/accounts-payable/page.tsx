...
const defaultForm: FormState = { vendor: '', amount: '', due: '', category: 'Technology' };
...
function validate(form: FormState): FormErrors {
  ...
  if (!form.due) {
    ...
    const picked = new Date(form.due);
    if (picked < today) {
...
export default function AccountsPayablePage() {
  ...
  const handleCreate = () => {
    setTouched({ vendor: true, amount: true, due: true });
    const errs = validate(form);
    setErrors(errs);
    if (Object.keys(errs).length > 0) return;

    const newBill = {
      invoice: `BILL-009${bills.length}`,
      vendor: form.vendor,
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white-800">Accounts Payable</h1>
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-gray-800 border-b border-gray-700">
              {['Invoice', 'Vendor', 'Amount', 'Due Date', 'Category', 'Status', 'Action'].map(h => (
          ...
          <tbody className="divide-y divide-gray-800">
            {bills.map(b => (
              <tr key={b.invoice} className="hover:bg-gray-800/60">
                ...
                <td className="px-4 py-3.5 text-gray-400 text-sm">{b.category}</td>
                <td className="px-4 py-3.5">
                  <span className={`px-2 py-0.5 rounded text-xs font-medium ${statusStyle[b.status]}`}>{b.status}</span>
                </td>
                <td className="px-4 py-3.5">
                  {b.status !== 'Paid' ? (
                    <button onClick={() => setPayBill(b.invoice)} className="px-3 py-1 bg-blue-600 text-white rounded text-xs hover:bg-blue-700">Pay Now</button>
      ...
      {/* Record Bill Modal */}
      <Modal open={showCreate} onClose={handleClose} title="Record New Bill">
        <div className="space-y-4">

          <FormField label="Vendor Name">
            <input
              className={errBorder(inputCls, !!(touched.vendor && errors.vendor))}
              placeholder="e.g. SupplierCo Ltd"
              value={form.vendor}
              onChange={e => handleChange('vendor', e.target.value)}
              onBlur={() => handleBlur('vendor')}
            />
            <Err msg={touched.vendor ? errors.vendor : undefined} />
          </FormField>
      ...
      <Modal open={!!payBill} onClose={() => setPayBill(null)} title="Confirm Payment" size="sm">
        <div className="space-y-4">
          <p className="text-gray-600 text-sm">
            Are you sure you want to process payment for <strong>{payBill}</strong>? This action will mark it as Paid.
          </p>
          <div className="flex gap-3">
            <button onClick={() => setPayBill(null)} className="flex-1 px-4 py-2 border border-gray-200 text-gray-600 rounded-lg text-sm hover:bg-gray-50">
              Cancel
            </button>
            <button onClick={() => handlePay(payBill!)} className="flex-1 px-4 py-2 bg-emerald-600 text-white rounded-lg text-sm font-medium hover:bg-emerald-700">
              Confirm Payment
            </button>
          </div>
        </div>
