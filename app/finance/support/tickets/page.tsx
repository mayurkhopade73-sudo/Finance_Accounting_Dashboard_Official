...
type FormState = { subject: string; client: string; priority: string; assignee: string };
type FormErrors = { subject?: string; client?: string };

const defaultForm: FormState = { subject: '', client: '', priority: 'Medium', assignee: 'Support Team' };
...
export default function TicketsPage() {
  ...
  const handleCreate = () => {
    setTouched({ subject: true, client: true });
    const errs = validate(form);
    setErrors(errs);
    if (Object.keys(errs).length > 0) return;

    setTickets(prev => [
      { id: `TKT-00${prev.length + 1}`, ...form, status: 'Open', created: 'Mar 13, 2026' },
      ...prev,
    ]);
    setForm(defaultForm);
    setErrors({});
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white-800">Support Tickets</h1>
          <p className="text-gray-500 text-sm mt-1">Track and resolve customer issues</p>
        </div>
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-100">
              {['ID', 'Subject', 'Client', 'Priority', 'Status', 'Created', 'Assignee', 'Actions'].map(h => (
            ...
            {tickets.map(t => (
              <tr key={t.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3.5">
                  <button onClick={() => setViewTicket(t)} className="p-1.5 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded transition-colors">
  ...
  <div className="space-y-4">
    ...
    <div className="flex items-center justify-between pt-2">
      ...
      <Modal open={showCreate} onClose={handleClose} title="New Support Ticket">
        <div className="space-y-4">

          <FormField label="Subject">
            <input
              className={errBorder(inputCls, !!(touched.subject && errors.subject))}
              value={form.subject}
          ...
          <div className="grid grid-cols-2 gap-3">
            <FormField label="Priority">
              <select className={selectCls} value={form.priority} onChange={e => handleChange('priority', e.target.value)}>
                <option>Low</option>
            ...
            <FormField label="Assignee">
              <select className={selectCls} value={form.assignee} onChange={e => handleChange('assignee', e.target.value)}>
                <option>Support Team</option>
                <option>Finance Manager</option>
        ...
        {viewTicket && (
          <div className="space-y-3">
            <div>
              ...
              <p className="text-gray-800 font-medium mt-0.5">{viewTicket.subject}</p>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div><span className="text-gray-500 text-xs uppercase">Client</span><p className="text-gray-700 mt-0.5">{viewTicket.client}</p></div>
              <div><span className="text-gray-500 text-xs uppercase">Assignee</span><p className="text-gray-700 mt-0.5">{viewTicket.assignee}</p></div>
            ...
            {viewTicket.status !== 'Resolved' && (
              <button onClick={() => handleResolve(viewTicket.id)} className="w-full mt-2 px-4 py-2 bg-emerald-600 text-white rounded-lg text-sm font-medium hover:bg-emerald-700">
                Mark as Resolved
              </button>
            )}
            {viewTicket.status === 'Resolved' && (
              <p className="text-emerald-600 text-sm text-center font-medium">✓ This ticket is resolved</p>
            )}
